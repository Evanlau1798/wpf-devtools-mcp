...
internal static class MutationBatchMcpToolDescriptions
{
    ...
    public const string BatchMutate =
        ...
        "DO NOT USE: When a single mutation call is sufficient, or when you have not captured an explicit snapshot for destructive experimentation.\n\n" +
        "SEQUENTIAL SEMANTICS: Mutations run in order and stop on the first failure. Set rollbackOnFailure=true with captureSnapshot to attempt restore_state_snapshot before the response returns.\n" +
        ...
        "If rollbackOnFailure=true and rollback.succeeded=true, the captured snapshot was restored before the response returned. " +
        "Otherwise, if captureSnapshot was provided and rollback.available=true, call restore_state_snapshot using rollback.params.snapshotId or recovery.params.snapshotId. " +
        ...
        "Failures include restore_state_snapshot guidance only while that snapshot remains retained; otherwise inspect rollback.available, rollback.attempted, rollback.succeeded, and recovery before retrying.\n" +
        ...
        "- captureSnapshot: optional object with elementId, propertyNames, viewModelPropertyNames, includeFocus, and snapshotName fields. Use captureSnapshot=true only for a single element batch where the server can infer propertyNames or viewModelPropertyNames from mutation args.\n" +
        "- includeDiff: optional boolean; set true only together with captureSnapshot when you need a post-batch get_state_diff.\n" +
        "- rollbackOnFailure: optional boolean; requires captureSnapshot and attempts restore_state_snapshot automatically after a non-timeout batch failure.\n" +
        ...
        "SCHEMA NOTES: Use nested `args`, not `arguments`, for mutation inputs. Nested args must not include processId; use the batch root processId only. includeDiff=true requires captureSnapshot so get_state_diff has a baseline snapshot. captureSnapshot=true infers a snapshot for single element set_dp_value, clear_dp_value, override_style_setter, or modify_viewmodel batches; provide an explicit captureSnapshot object for multi-element batches or bound source rollback.\n\n" +
        ...
        "- { \"processId\": 12345, \"captureSnapshot\": true, \"includeDiff\": true, \"mutations\": [{ \"tool\": \"set_dp_value\", \"args\": { \"elementId\": \"NameTextBox\", \"propertyName\": \"Text\", \"value\": \"Alice\" } }] }\n" +
        ...
        "  - rollback (optional): { available: boolean, snapshotId (optional), tool (optional), params (optional), rollbackOnFailure (optional), attempted (optional), succeeded (optional), result (optional), reason (optional) },\n" +
