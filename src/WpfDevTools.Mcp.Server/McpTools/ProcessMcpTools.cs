...
public static class ProcessMcpTools
{
    ...
    [Description(
        ...
        "POLICY PRECONDITION: WPFDEVTOOLS_MCP_ALLOWED_TARGETS scopes discovery to allowlisted targets only; targets denied by policy are counted but their process name, window title, executable path, architecture, runtime, and elevation metadata are not disclosed.\n" +
    ...
    [Description(
        ...
        "POLICY PRECONDITION: WPFDEVTOOLS_MCP_ALLOWED_TARGETS must contain the reviewed target's exact local absolute executable path before this tool can attach; unset or malformed values fail closed with SecurityError.\n" +
