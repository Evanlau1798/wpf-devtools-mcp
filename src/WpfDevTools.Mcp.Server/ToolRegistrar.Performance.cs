...
public static partial class ToolRegistrar
{
    ...
    private static void RegisterPerformanceTools(ToolRegistry registry, SessionManager sessionManager)
    {
        RegisterTool(registry, "get_render_stats",
            "[Performance] Get render statistics from a WPF application. Returns frame rate, render time, dirty region count, and other WPF rendering pipeline metrics. Use as a first step when investigating slow UI. Returns: { frameRate, avgRenderTime, dirtyRegionCount, visualCount }",
        ...
        RegisterTool(registry, "find_binding_leaks",
            "[Performance] Detect potential binding memory leaks by tracking live binding references. Threshold is the minimum number of live bindings on a single element to flag as suspicious (default: 100). Returns: { suspects: [{ elementId, elementType, bindingCount }] }",
        ...
        RegisterTool(registry, "measure_element_render_time",
            "[Performance] Measure the render time of a WPF element in milliseconds. Forces a re-render and measures the time taken. Use to identify slow-rendering elements. Returns: { renderTimeMs, elementId }",
        ...
        RegisterTool(registry, "get_visual_count",
            "[Performance] Get the count of visual elements in a WPF element subtree. High counts (>5000) may indicate performance issues. Use to identify overly complex UI subtrees. Returns: { count, elementId }",
            new { type = "object", properties = new { processId = new { type = "integer", description = "Process ID of the connected WPF application (from get_processes)" }, elementId = new { type = "string", description = "Element ID obtained from get_visual_tree or get_logical_tree. Omit to target root window." } }, required = new[] { "processId" } },
