...
請先審查維護者來源：[scripts/online-installer.ps1](https://github.com/Evanlau1798/wpf-devtools-mcp/blob/master/scripts/online-installer.ps1)。這支已審查的 installer 會解析對應的 published release asset、在解壓前驗證 archive integrity，然後透過已審查的 installer/helper flow 安裝解壓出的 packaged payload。它會下載已發布的 GitHub Release package，不會從原始碼建置。
...
Repository 內的 entrypoint 仍只是 bootstrap 層；實際安裝動作會透過已審查的 installer/helper flow 使用已驗證且解壓出的 release payload。
