...
- Pack 自有的 `wpfui.editorialCard` block 提供含媒體、文案、任意內容與操作 slots 的橫向 editorial surface。其 optional `mediaSource` 使用 WPF `binding`；不設定它並填入 optional media slot，即可建立具 accessibility 資訊的 symbol fallback。這仍是 pack data，具有 no Composer engine special case 的特性，因此第三方 pack 可用自己的 contracts 與 renderers 提供同等媒體模式。
