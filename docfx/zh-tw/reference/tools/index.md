...
| 探索 Composer packs | `list_ui_block_packs` | catalog 支援可用後再接 `get_ui_block_catalog` | 在 blueprint 工作前列出 built-in、project-local 與 user-global UI block packs。 |
...
| UI Composer | `list_ui_block_packs` | 在 composition 前探索可用 UI block packs 與 provenance |
