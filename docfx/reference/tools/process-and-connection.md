...
- Use `get_processes(windowFilter)` when auto-discovery is ambiguous, when you need architecture/elevation details up front, or when you want to inspect background targets. It returns details only for allowlisted targets; each denied target is counted in `redactedTargetCount` without exposing process names, titles, paths, architecture, runtime, or elevation metadata.
...
- Successful `connect` responses include `connectionSource`: `active-session` when the server reused an already-connected session, `sdk-hosted-inspector` when it reused an existing target-side Inspector host, or `raw-injection` when it attached through the packaged injector/bootstrapper path.
- After a raw-injection attach, the target may keep an injected Inspector host alive. Later server sessions against the same still-running target can legitimately report `connectionSource: "sdk-hosted-inspector"` because they are reusing that host. Restart the target process before a validation run that needs to prove first raw-injection attach provenance.
