...
- `compactRuntimeDiagnostics`: optional boolean that defaults to true. Compact mode omits successful generated XAML and risk-free correlation details while keeping their lengths/counts, each tool outcome, failed diagnostic payload, screenshot resource handles, and any correlations needed to investigate layout risk. Set false only when the complete XAML, correlation, and raw diagnostic payloads are required.
