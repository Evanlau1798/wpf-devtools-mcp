...
| 驗證 Composer blueprints | `list_ui_block_packs` | `get_ui_block_catalog`、`validate_ui_blueprint` | 先列出 installed UI packs、檢查 composition rules，再於 rendering 或 apply workflow 前驗證 blueprint JSON。 |
...
| UI Composer | `list_ui_block_packs`、`get_ui_block_catalog`、`validate_ui_blueprint` | 在 rendering 前探索 UI block packs、檢查 composition rules 並驗證 blueprints |
