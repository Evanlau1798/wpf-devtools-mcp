...
3. 執行 package-local installer 時，請把已驗證的 release zip、`SHA256SUMS.txt`、`release-assets.json` 與 `release-sbom.spdx.json` 一起保留在解壓資料夾的父目錄。SBOM sidecar 是 asset-level release archive inventory，not a full package/dependency SBOM。否則就必須明確提供 `WPFDEVTOOLS_RELEASE_SIGNER_THUMBPRINT` 作為必要的 thumbprint trust root；`WPFDEVTOOLS_RELEASE_SIGNER_SUBJECT` 只能在 thumbprint 已 pin 之後作為 additional constraint。
4. 解壓縮套件。
