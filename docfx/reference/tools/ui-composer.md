...
- `runtimePackApprovalTokens`: up to 16 reviewed content-bound tokens returned by `PreviewRuntimeDependenciesNotApproved`. This requires `WPFDEVTOOLS_MCP_ALLOW_COMPOSER_RUNTIME_APPROVALS=true`; each token applies to one preview call and is never persisted.
