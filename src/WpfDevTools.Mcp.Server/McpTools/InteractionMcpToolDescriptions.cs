...
internal static class InteractionMcpToolDescriptions
{
    ...
    public const string GetFocusState =
        ...
        InteractionMetadata + "Get the current logical or keyboard focus snapshot for a window or element scope.\n\n" +
    ...
    public const string FocusElement =
        ...
        InteractionMetadata + "Move logical focus to a specific WPF element.\n\n" +
    ...
    public const string DragAndDrop =
        ...
        InteractionMetadata + "Simulate drag and drop between two WPF elements. " +
    ...
    public const string ScrollToElement =
        ...
        InteractionMetadata + "Scroll a WPF element into view within its parent ScrollViewer. " +
    ...
    public const string SimulateKeyboard =
        ...
        InteractionMetadata + "Simulate a keyboard key press on an element. " +
    ...
    public const string ElementScreenshot =
        ...
        InteractionMetadata + "Capture a PNG screenshot of a specific element. " +
        ...
        "PRIVACY: Enable the MCP screenshot policy gate. For larger captures use `outputMode: \"file\"`; it returns an MCP server-owned retained screenshot resource under a server-issued lease root, with redacted local paths, a session-scoped `resourceUri`, and the exact same-session `resourceRead`. Do not pass `outputPath`. If the full blob is truncated, assemble the advertised `resourceRead.chunking` ranges (at most 16 KiB each), then verify `byteLength` and `sha256`. Resources expire after 24 hours, cap at 100, and are deleted on eviction, expiry, disconnect, or disposal. Retention is owned by `SessionManager`, not by the Inspector default screenshot cache. Use inline `base64` only for small images.\n" +
        "PERFORMANCE: The default `metadata` mode does not render or return PNG bytes; metadata mode does not return `screenshotId`, `resourceUri`, or a `wpf://screenshots/{screenshotId}` handle. Metadata responses include a nextSteps entry for `outputMode: \"file\"` when pixel evidence is required. Use size limits for compact evidence; omit both for typography or pixel judgment, then verify sourceWidth and downscaled=false.\n\n" +
        "RESPONSE SUMMARY:\n" +
        "  - success: boolean,\n" +
        ...
        "- \"outputPath\" -> omit it; use outputMode file and read resourceUri instead\n" +
