...
- `WPFDEVTOOLS_COMPOSER_TRUSTED_RUNTIME_PACKS` 只接受由 operator 控制的 server-start 設定所提供的已審查 content-bound token。每個 token 都綁定精確 pack root、id、version 與 fingerprint；MCP caller 無法在 tool call 中核准 runtime dependency，且 preview 仍需要 `WPFDEVTOOLS_MCP_ALLOW_DESTRUCTIVE_TOOLS=true`。
