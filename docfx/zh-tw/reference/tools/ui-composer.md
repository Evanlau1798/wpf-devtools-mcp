...
- `compactRuntimeDiagnostics`: optional boolean，預設為 true。Compact mode 會保留每個 tool outcome、失敗診斷的 payload 與 screenshot resource handle，同時省略 successful non-screenshot payloads；bounded layout 與 correlation coverage 仍保留在 `layoutRiskSummary`。只有需要完整 raw diagnostic payloads 時才設為 false。
