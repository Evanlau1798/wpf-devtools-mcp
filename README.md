...
- Set `WPFDEVTOOLS_MCP_ALLOW_COMPOSER_RUNTIME_APPROVALS=true` only when reviewed content-bound tokens may be supplied to `preview_ui_blueprint` for one request. `WPFDEVTOOLS_COMPOSER_TRUSTED_RUNTIME_PACKS` remains the operator-controlled server-start option, and preview still requires the destructive-tools gate.
