...
When runtime diagnostics are enabled, `layoutRiskSummary` maps clipped correlated elements back to exact blueprint paths without relying on pack-specific kinds or slot names. It reports the total clipped-element count, at most 32 warnings, truncation state, block kind, element identity, clipping source, directional overflow, and the runtime `suggestedFix`. Treat these as early structural risks to repair before apply, then repeat clipping checks in the final built application because extension-package templates can measure differently from preview stubs.
