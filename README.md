...
- Configure reviewed content-bound tokens in operator-controlled `WPFDEVTOOLS_COMPOSER_TRUSTED_RUNTIME_PACKS` before starting or restarting the server. MCP callers cannot approve third-party runtime dependencies in a tool call, and preview still requires the destructive-tools gate.
