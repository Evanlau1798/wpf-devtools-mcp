...
3. 執行 package-local installer 時，請把已驗證的 release zip、`SHA256SUMS.txt` 與 `release-assets.json` 一起保留在解壓資料夾的父目錄；否則就必須明確提供 `WPFDEVTOOLS_RELEASE_SIGNER_THUMBPRINT` 與 `WPFDEVTOOLS_RELEASE_SIGNER_SUBJECT`。
4. 解壓縮套件。
