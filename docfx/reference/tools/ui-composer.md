...
- `compactRuntimeDiagnostics`: optional boolean that defaults to true. Compact mode keeps each tool outcome, failed diagnostic payload, and screenshot resource handle while omitting successful non-screenshot payloads; bounded layout and correlation coverage remain in `layoutRiskSummary`. Set false only when the complete raw diagnostic payloads are required.
