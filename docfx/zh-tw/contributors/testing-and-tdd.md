...
- `HostedWindowsX64` 會在 Windows Sandbox 可可靠執行的範圍內貼近 GitLab Windows x64 fallback lane 與 GitHub hosted Windows CI/CD 範圍：精確的 x64 與 x86 native bootstrapper builds、Debug/Release x64 與 x86 solution builds、x64 unit shards、x64 release-unit shards，以及 x64 Debug integration tests。它涵蓋 coverage、x64/x86/arm64 release packaging smoke、NuGet pack、ARM64 Release cross-build，以及本機 DocFX Pages build steps。它仍不涵蓋 x86 test execution 或 self-hosted ARM64 runtime smoke lanes；也不模擬 GitHub Pages upload/deployment、GitHub artifact upload/download 邊界、已簽章 public release publication，或 GitHub-hosted runner image 差異。
- x86 與 arm64 packaging smoke 會遵循 GitHub hosted x64 行為：執行 package-local 與 online installer 的 install/uninstall layout checks，但不啟動非 x64 packaged server runtime，因為 hosted x64 lane 無法執行這些 runtime smoke。每一組 install/uninstall 都使用隔離的 `APPDATA` 與 `LOCALAPPDATA` root，避免 package-local 與 online installer state 互相遮蔽。
