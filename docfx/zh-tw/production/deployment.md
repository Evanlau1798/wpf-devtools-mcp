...
- 使用 `-Action full-uninstall` 搭配 `-InstallRoot <exact-root>` 時，只會移除該 root 下的 registrations、generated client-registration artifacts 與 installer-owned server payloads。只有測試執行後或 decommissioning workflow 需要移除所有 detected installer roots 時，才省略 `-InstallRoot`。
