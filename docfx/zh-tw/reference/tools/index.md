...
DocFX validation script 會依 source tree 中的 `[McpServerTool]` attributes 驗證 tool-name coverage，並檢查 listed tool names 不過期。Unit documentation tests 會驗證下方 generated contract snapshot hashes。Validation script 不會重新產生或完整驗證 parameter list、output schema 或 capability tags。
