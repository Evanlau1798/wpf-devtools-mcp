...
STDIO request 的 raw MCP JSON-RPC envelope 會先由 MCP C# SDK 解析，server 之後才會收到 typed request。因此 `initialize`、`resources/read`、`tools/list` 上的 `id` 與 `method` 等 pre-dispatch envelope fields 是 SDK-owned。本專案在 SDK parsing 之後驗證 tool-call names and arguments，接著在把 request 派送到 injected host 前，驗證 Inspector IPC request ids, methods, and correlation ids。
...
Screenshot capture 另外受 resource lifecycle controls 限制。`element_screenshot` 預設只回傳 metadata。Inline `base64` 只允許小型 PNG payload；較大的 pixel capture 必須使用 `outputMode: "file"`，讓回應提供 `wpf://screenshots/{screenshotId}` resource handle，而不是本機路徑。File-mode resource 會在 24 小時後到期、每個 MCP server session 最多保留 100 張，被淘汰或到期時會刪除 retained PNG file，並會在 target session disconnect 或 server session manager dispose 時清除。`full-uninstall` 也會移除預設目前使用者的 screenshot cache：`%LOCALAPPDATA%\WpfDevTools\tmp\screenshots`；auth secrets 與 certificates 仍維持需要手動刻意清理。
