...
internal static partial class McpToolOutputSchemas
{
    ...
    private static Dictionary<string, JsonElement> CreateHighValueSchemas()
        => new(StringComparer.Ordinal)
        {
            ...
            ["element_screenshot"] = CreateSchema(false, new()
            {
                ...
                ["outputMode"] = EnumString("Selected screenshot output mode; metadata mode intentionally returns no pixel bytes, while file mode plus resources/read is required for normal pixel evidence.", ["metadata", "file", "base64"]),
                ["width"] = Integer("Screenshot width in pixels."),
                ["height"] = Integer("Screenshot height in pixels."),
                ["sourceWidth"] = Integer("Rendered source width before optional downscaling."),
                ["sourceHeight"] = Integer("Rendered source height before optional downscaling."),
                ["scaleFactor"] = Number("Applied screenshot scale; 1 means source-sized pixels."),
                ["downscaled"] = Boolean("Whether pixels were downscaled and are therefore unsuitable as authoritative typography evidence."),
                ["format"] = String("Screenshot image format label."),
                ["rendered"] = Boolean("Whether screenshot pixels were rendered; false for metadata mode is intentional."),
                ["byteLength"] = Integer("Screenshot payload byte length; zero for metadata mode because no pixel bytes are produced."),
    ...
    private static Dictionary<string, object> CommonProperties()
        => new(StringComparer.Ordinal)
        {
            ...
            ["pendingEventsMayIncludePriorContext"] = Boolean("Whether events may predate this request."),
            ["pendingEventsSuggestedAction"] = String("drain_events guidance for a clean event window."),
            ["pendingEventsAreAdvisory"] = Boolean("True when piggybacked events may predate this request and are not causal evidence."),
            ["pendingEventsSummary"] = String("Event count/types plus prior-context and clean-drain guidance."),
            ["pendingEventsPiggybackFailed"] = Boolean("True when event piggyback failed after the primary result succeeded."),
            ["pendingEventsPiggybackFailureType"] = String("Event piggyback failure type."),
            ["pendingEventsMayRemainBuffered"] = Boolean("Whether events may remain for drain_events."),
            ["pendingEventsPiggybackRequiresReconnect"] = Boolean("Whether reconnect is needed before event recovery."),
            ["pendingEventsStateAfterTimeoutUnknown"] = Boolean("Whether piggyback timeout/reset left event state unknown."),
            ["pendingEventsPiggybackSuggestedAction"] = String("Recommended recovery workflow after piggyback failure."),
            ["restoreRequired"] = Boolean("Whether to restore the mutation to leave the app unchanged."),
            ["restoreStatus"] = String("Restore status for the mutation response, such as notRestored."),
            ["restoreSuggestedAction"] = String("restore_state_snapshot guidance after verification."),
