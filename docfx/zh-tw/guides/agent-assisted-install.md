...
4. 若已驗證的 sidecar 不再與 package 相鄰，使用 `WPFDEVTOOLS_RELEASE_SIGNER_THUMBPRINT` 作為 exact certificate thumbprint pin。
5. 只有在使用者明確接受 subject-based policy 時，才使用 `WPFDEVTOOLS_RELEASE_SIGNER_SUBJECT`。
