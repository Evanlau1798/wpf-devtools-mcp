...
`find_elements` also applies a traversal cap before evaluating matches: `maxTraversalNodes` defaults to `1000` and is capped at `10000`. The optional `query` parameter is a bounded convenience search over common semantic fields such as element type, `FrameworkElement.Name`, AutomationId, Text, Content, and Header; use exact filters like `typeName`, `elementName`, or `automationId` when the automation path must be deterministic. Type filters use `typeMatchMode="exact"` by default. Set `typeMatchMode="assignable"` to match the concrete WPF type, its base types, or implemented interfaces by simple or fully qualified name. The independent `matchMode` still controls exact versus case-insensitive substring comparison, including combinations such as `typeMatchMode="assignable"` with `matchMode="contains"`. When a zero-result search returns `traversalTruncated=true`, `searchComplete=false` means the result is inconclusive. Inspect `traversalNodeCount`, then follow the structured `TraversalBudgetExceededBeforeMatch` recovery: retry its suggested `maxTraversalNodes` value, or use the `elementId` guidance to choose a narrower root before raising the cap further.
