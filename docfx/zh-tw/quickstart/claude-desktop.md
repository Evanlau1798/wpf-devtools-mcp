...
- `WPFDEVTOOLS_MCP_ALLOWED_TARGETS` 已包含已審查 target 的 exact absolute executable path 後，一般情況先從 `connect()` 開始；只有 auto-discovery 回報多個候選時，才使用 `get_processes(windowFilter)`。
