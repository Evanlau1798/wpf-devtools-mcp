...
- `runtimePackApprovalTokens`: 最多 16 個由 `PreviewRuntimeDependenciesNotApproved` 回傳且已審查的 content-bound tokens。需要 `WPFDEVTOOLS_MCP_ALLOW_COMPOSER_RUNTIME_APPROVALS=true`；每個 token 只適用於一次 preview call，而且永不持久化。
