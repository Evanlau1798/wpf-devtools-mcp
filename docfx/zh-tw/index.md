...
手動驗證安裝是已審查 archive 的獨立路徑。請把 `SHA256SUMS.txt`、`release-assets.json`、`release-sbom.spdx.json` 與 `package-sbom.spdx.json` 放在 archive 旁邊，然後依照 [手動驗證安裝](quickstart/manual-install.md) 操作。
...
| 從已審查本機 archive 安裝 | [手動驗證安裝](quickstart/manual-install.md) |
| 註冊 MCP client | [AI Agent Client](quickstart/ai-agent-clients.md) |
| 讓 agent 取得 runtime workflow contract | [AI Agent 使用指南](guides/ai-agent-guide.md) |
| 依照穩定任務流程操作 | [常見工作流程](guides/common-workflows.md) |
| 先釐清專案常用詞 | [詞彙表](reference/glossary.md) |
| 查看 MCP resources、prompts 與 navigation fields | [MCP Contracts 與 Navigation](reference/mcp-contracts.md) |
