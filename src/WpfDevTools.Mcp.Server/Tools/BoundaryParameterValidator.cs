...
internal static class BoundaryParameterValidator
{
    ...
    {
        error = null;
        if (BoundaryJsonStringValidator.TryValidate(arguments, out var validationError))
        {
            return true;
        }

        error = CreateInvalidArgument(
            $"{validationError.DisplayName} exceeds the maximum length of {validationError.MaxLength} characters.",
            $"Shorten {validationError.DisplayName}; oversized strings are rejected before the request is forwarded to the target process.");
        return false;
    }
