...
internal static class InteractionMcpToolDescriptions
{
    ...
    public const string ElementScreenshot =
        ...
        "PRIVACY: The MCP screenshot policy gate must be enabled. Use `outputMode: \"file\"` for larger pixel captures; it returns a session-scoped `resourceUri` plus an exact `resourceRead` request, redacts local paths, and creates an MCP server-owned retained screenshot resource under a server-issued lease root. Do not pass `outputPath`; file mode is resource-backed and clients should call the returned `resourceRead.method` with `resourceRead.params` in the same MCP server session. If the client truncates the complete blob, read and assemble the bounded ranges advertised by `resourceRead.chunking`, then verify the original `byteLength` and `sha256`. `SessionManager` expires the resource after 24 hours, caps it at 100 resources per MCP server session, deletes retained PNG files when evicted or expired, and purges them on disconnect or server session manager disposal. This lifecycle is owned by `SessionManager`, not by the Inspector default screenshot cache. Inline `base64` is capped for small images only.\n" +
