...
6. 解壓後，使用原始 archive 與 metadata 目錄執行 packaged installer：
...
一般 client registration 建議使用 `<InstallRoot>\<arch>\current\bin\wpf-devtools-<arch>.exe` 內的 installed executable。若是不安裝的 portable 驗證，只有在解壓後 package 與原始 archive 及 `SHA256SUMS.txt` 放在同一目錄，且 manifest、executable、inspector、bootstrapper payload bytes 仍符合已驗證 ZIP 時，才使用 package-local `run.bat` 或 `bin\wpf-devtools-<arch>.exe`。
...
- 如果手動 package 安裝後 `connect()` 回傳 `SecurityError: Security verification failed`，請先確認 `<InstallRoot>\<arch>\current\bin\` 下的 installed path。若使用 package-local `run.bat` 或 `bin\wpf-devtools-<arch>.exe`，請確認解壓後 package 與原始 archive 及 `SHA256SUMS.txt` 放在同一目錄；否則請改用 packaged installer path。
