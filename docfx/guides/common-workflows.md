...
6. If the current session has buffered runtime events, `drain_events` is the first explicit event-verification step
7. Without a snapshot, use `get_interaction_readiness`, `get_element_snapshot`, `get_dp_value_source`, or scoped `get_ui_summary` to verify the effect
...
3. Apply one mutation such as `set_dp_value`, `modify_viewmodel`, or `override_style_setter`, or use `batch_mutate` for an ordered sequence
4. Call `get_state_diff` first
5. Call `drain_events` when you need an explicit read of buffered binding, DP, or validation events
6. If the tool response offers a more specific `navigation.recommended`, use it for supplementary verification
7. `restore_state_snapshot` if the app should return to its original state
