...
請註冊 `<InstallRoot>\<arch>\current\bin\wpf-devtools-<arch>.exe` 內的 installed executable。不要把 client 指到解壓 archive 裡的 package-local executable；pre-release checksum trust 會由 installer 搭配 trusted sidecars 解析。
...
- 如果手動 package 安裝後 `connect()` 回傳 `SecurityError: Security verification failed`，請確認 client 指向 `<InstallRoot>\<arch>\current\bin\` 下的 installed path，而不是解壓 archive 中的 package-local executable。
