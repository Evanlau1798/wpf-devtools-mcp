...
| 展開 Composer recipes | `list_ui_block_packs` | `get_ui_block_catalog`、`expand_ui_recipe`、`validate_ui_blueprint` | 先列出 installed UI packs 與 recipes、展開 starter recipe，再於 rendering 或 apply workflow 前驗證 blueprint JSON。 |
...
| UI Composer | `list_ui_block_packs`、`get_ui_block_catalog`、`expand_ui_recipe`、`validate_ui_blueprint` | 在 rendering 前探索 UI block packs、檢查 recipes 與 composition rules，並展開及驗證 blueprints |
