...
啟用 runtime diagnostics 時，`layoutRiskSummary` 會把遭裁切的 correlated elements 對應回精確 blueprint paths，不依賴 pack-specific kinds 或 slot names。Coverage 會明確回報：`correlatedTargetCount` 是 distinct renderer correlations 數量、`resolvedTargetCount` 是 preview tree 中找到的數量、`inspectedTargetCount` 是 `get_clipping_info` 成功檢查的數量。`inspectionTruncated=true` 表示 coverage 不完整；`warningsTruncated` 則只代表另一個最多 32 筆 warnings 的輸出上限。Summary 也會回報 clipped-element 總數、block kind、element identity、clipping source、各方向 overflow 與 runtime `suggestedFix`。請在 apply 前把這些項目當成早期結構風險修復；由於 extension-package templates 的實際量測可能不同於 preview stubs，final built application 仍須重做 clipping checks。
