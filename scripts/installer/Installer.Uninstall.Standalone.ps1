    ...
    foreach ($registration in @(Get-StandaloneDetectedConfigRegistrations)) {
        ...
        if ($registrationMap.Contains($stateKey)) {
            ...
            if ($existingTargetHasRegistration) {
                $existingLiveExecutable = Get-JsonConfigRegisteredExecutable -CollectionName $collectionName -ConfigPath $existingTarget
                $existingLiveOwnership = Resolve-InstallerOwnershipFromExecutable -InstalledExecutable $existingLiveExecutable
            ...
            if ($existingTargetHasRegistration -and $liveTargetHasRegistration -and
                -not (Test-InstallerPathEqualsCore -Left $existingTarget -Right $liveTarget)) {
            ...
            $preferLiveEvidence = $liveTargetHasRegistration -and (
                ...
                (Test-InstallerPathEqualsCore -Left $existingTarget -Right $liveTarget))
