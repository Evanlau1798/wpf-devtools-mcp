...
internal static class UiComposerMcpToolDescriptions
{
    ...
    public const string PreviewUiBlueprint =
        ...
        - Built-ins have release provenance. Project/user packs remain structural until runtimePackApprovalReviews is inspected and its content-bound approvalToken is set in WPFDEVTOOLS_COMPOSER_TRUSTED_RUNTIME_PACKS, or passed in runtimePackApprovalTokens for one preview call after WPFDEVTOOLS_MCP_ALLOW_COMPOSER_RUNTIME_APPROVALS=true. Each review includes pack identity, scope, fingerprint, selected resources, exact package closure, approval scope, and state. The full package closure requires exact [version] plus SHA-512 contentHash, restores to a preview-local NuGet cache, and is hash-checked before build.
