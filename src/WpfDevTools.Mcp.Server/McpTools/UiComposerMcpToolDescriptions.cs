...
internal static class UiComposerMcpToolDescriptions
{
    ...
    public const string PreviewUiBlueprint =
        ...
        - layoutRiskSummary maps Window client clipping to jsonPath/blockKind. Reasons: ambiguous-authored-name, lookup-budget, runtime-match-ambiguous, runtime-not-found, search-incomplete. After a complete lookup, get_namescope marks registered targets absent from visual search as namescope-only under namescopeOnlyCorrelationCount; inactive or lazy content alone does not set inspectionTruncated. RuntimeStructuralOverflowRisk uses structural-overflow and visibleContentRisk=unconfirmed-structural; RuntimeClippingDetected uses riskClassification=clipping and visibleContentRisk=unconfirmed-clipping. advisory with visibleContentImpact=not-determined and requiresVisualConfirmation=true means confirm pixels.
