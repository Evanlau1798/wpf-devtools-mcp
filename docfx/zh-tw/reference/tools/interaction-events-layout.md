...
對 `focus_element` 與 `simulate_keyboard`，請從 active rendered visual tree 中選擇 visible、enabled、focusable 的控制項。若某個 real-project target 回 `ElementNotLoaded` 或無法取得鍵盤焦點，先用 `get_interaction_readiness` 或 `get_element_snapshot` 檢查，再換另一個已載入且可 focus 的候選元素重試，不要只因單一候選失敗就判定 workflow 有限制。
